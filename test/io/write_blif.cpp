...
template<class Ntk>
void blif_multiple_definition_test( const Ntk& to_write )
{
  std::ostringstream out;
  write_blif_params ps;
  ps.skip_feedthrough = true;
...
TEST_CASE( "write a sequential k-LUT with a node driving both PO and RI (skip feedthrough = false)", "[write_blif]" )
{
  /**
   * 
   *              f4 (PO)
...
TEST_CASE( "write a sequential k-LUT with a node driving both PO and RI (skip feedthrough = true)", "[write_blif]" )
{
  /**
   * 
   *              f4 (PO)
